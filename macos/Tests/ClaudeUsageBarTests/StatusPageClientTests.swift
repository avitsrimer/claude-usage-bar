...
/// Tests for `StatusPageClient` decoding + error mapping. All network is faked via
/// `FixtureHTTPClient` so no live calls hit `status.claude.com` during tests or CI.
final class StatusPageClientTests: XCTestCase {
    ...
    // MARK: - End-to-end via StatusMonitor for outage/maintenance/unknown fixtures

    func testMonitorRefreshWithMajorOutageFixtureRollsUpToMajorOutage() async throws {
        let data = try loadFixture("statuspage_summary_major_outage_with_incident")
        let client = StatusPageClient(http: FixtureHTTPClient(status: 200, body: data))
        let monitor = await StatusMonitor(client: client, clock: ImmediateTestClock(), notificationCenter: NotificationCenter())
        await monitor.refresh()
        let rollup = await monitor.snapshot?.rollup
        XCTAssertEqual(rollup, .majorOutage)
        let incidentCount = await monitor.snapshot?.activeIncidents.count
        XCTAssertEqual(incidentCount, 1)
    }

    func testMonitorRefreshWithUnderMaintenanceFixtureTreatsAsOperational() async throws {
        let data = try loadFixture("statuspage_summary_under_maintenance")
        let client = StatusPageClient(http: FixtureHTTPClient(status: 200, body: data))
        let monitor = await StatusMonitor(client: client, clock: ImmediateTestClock(), notificationCenter: NotificationCenter())
        await monitor.refresh()
        // underMaintenance has severity 0, same as operational, so the rollup stays .operational
        // even though a monitored component reports under_maintenance.
        let rollup = await monitor.snapshot?.rollup
        XCTAssertEqual(rollup, .operational)
        let impactedCount = await monitor.snapshot?.impactedComponents.count
        XCTAssertEqual(impactedCount, 0)
    }

    func testMonitorRefreshWithUnknownStatusFixtureDoesNotSurfaceAnError() async throws {
        let data = try loadFixture("statuspage_summary_unknown_status_string")
        let client = StatusPageClient(http: FixtureHTTPClient(status: 200, body: data))
        let monitor = await StatusMonitor(client: client, clock: ImmediateTestClock(), notificationCenter: NotificationCenter())
        await monitor.refresh()
        let lastError = await monitor.lastError
...
/// Fast clock that returns immediately for every sleep. Shared with StatusMonitorTests'
/// equivalent double under a distinct name to avoid a cross-file redeclaration.
struct ImmediateTestClock: StatusClock {
